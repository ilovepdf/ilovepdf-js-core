...
describe('SignTask', () => {
    ...
    it('saves task as template', () => {
        ...
        .then(data => {
            expect(data.template_name).toBe('This is my template');
        });
    });
    ...
    it('process from template', () => {
        ...
        .then(async (template) => {
            ...
            return signTaskFromTemplate.processFromTemplate(template);
        })
    ...
    it('process in batch mode from template', () => {
        const task = taskFactory.newTask('sign', auth, xhr) as SignTask;
        ...
        .then(() => {
            ...
            return ILovePDFCoreApi.getSignatureTemplate(auth, xhr, task.id);
        })
        .then(async (template) => {
            const signTaskFromTemplate = taskFactory.newTask('sign', auth, xhr) as SignTask;
            await signTaskFromTemplate.start();
    ...
    it('updates a signer phone', async () => {
        const task = taskFactory.newTask('sign', auth, xhr) as SignTask;
    ...
    it('updates a signer email', async () => {
        const task = taskFactory.newTask('sign', auth, xhr) as SignTask;
        ...
        .then(() => {
            const signer = task.signers[0];
            return signer.updateEmail('email@falso.com');
        })
        .catch(err => {
    ...
    it('updates a signer status', async () => {
        const task = taskFactory.newTask('sign', auth, xhr) as SignTask;
    ...
    it('gets statuses', async () => {
        ...
        .then(status => {
            expect(status.document).toBe('sent');
        });
    });
